<?php
  ...
  $conf['servers'][0]['desc'] = '{desc}';

  // Hostname or IP address for server.  Use '' for UNIX domain socket.
  // use 'localhost' for TCP/IP connection on this computer
  $conf['servers'][0]['host'] = '{host}';
  ...
  $conf['servers'][0]['sslmode'] = '{sslmode}';

  // Change the default database only if you cannot connect to template1.
  // For a PostgreSQL 8.1+ server, you can set this to 'postgres'.
  $conf['servers'][0]['defaultdb'] = '{defaultdb}';

  // Specify the path to the database dump utilities for this server.
  // You can set these to '' if no dumper is available.
  $conf['servers'][0]['pg_dump_path'] = '{pg_dump_path}';
  $conf['servers'][0]['pg_dumpall_path'] = '{pg_dumpall_path}';
  ...
  $conf['default_lang'] = '{default_lang}';
  ...
  $conf['extra_login_security'] = {extra_login_security};

  // Only show owned databases?
  // Note: This will simply hide other databases in the list - this does
  // not in any way prevent your users from seeing other database by
  // other means. (e.g. Run 'SELECT * FROM pg_database' in the SQL area.)
  $conf['owned_only'] = {owned_only};

  // Display comments on objects?  Comments are a good way of documenting
  // a database, but they do take up space in the interface.
  $conf['show_comments'] = {show_comments};

  // Display "advanced" objects? Setting this to true will show 
  // aggregates, types, operators, operator classes, conversions, 
  // languages and casts in phpPgAdmin. These objects are rarely 
  // administered and can clutter the interface.
  $conf['show_advanced'] = {show_advanced};

  // Display "system" objects?
  $conf['show_system'] = {show_system};

  // Minimum length users can set their password to.
  $conf['min_password_length'] = {min_password_length};
  ...
  $conf['show_oids'] = {show_oids};
  
  // Max rows to show on a page when browsing record sets
  $conf['max_rows'] = {max_rows};

  // Max chars of each field to display by default in browse mode
  $conf['max_chars'] = {max_chars};
  ...
  $conf['ajax_refresh'] = {ajax_refresh};
